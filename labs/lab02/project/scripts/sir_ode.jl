...
using Plots
# Явно указываем формат графиков
if isdefined(Main, :Quarto) || isdefined(Main, :IJulia)
    # В среде Quarto/IJulia используем PNG
    Plots.gr(size=(800,500), fmt=:png)
else
    # В обычном запуске можно оставить по умолчанию
    nothing
end
